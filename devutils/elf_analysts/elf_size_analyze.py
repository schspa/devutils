...
sections must have ALLOC flag and: for RAM - have WRITE flag, for ROM - not have NOBITS type.
    ...
    printing_group.add_argument('--show-treemap', action='store_true',
                                help='show treemap with dash')
...
class SymbolsTreeByPath:
    ...
    class Node(TreeNode):
        ...
        def name(self):
            return self.data.name if self.is_symbol() else self.data
...
def main():
    ...
    def show_treemap(header, symbols):
        items = {}
        items['size'] = []
        path = []
        maxdepth, itemdicts = get_items_dicts(header, symbols)
        ...
        for item in itemdicts:
            ...
            for i in range(maxdepth + 1):
                items[i].append(item[i] if i in item else None)
                pass
        df = pd.DataFrame(items)
        fig = px.treemap(df, path=path, values = 'size')
        return fig

    def get_items_dicts(header, symbols):
        tree = SymbolsTreeByPath(symbols)
        if not args.no_merge_paths:
            tree.merge_paths(args.fish_paths)
        if not args.no_cumulative_size:
...
ERROR: No symbols from given section found or all were ignored!
    ...
    if args.rom:
        ...
        if args.show_treemap:
            fig = show_treemap('ROM', filter_symbols(lambda sec: sec.occupies_rom()))
            figs.append(fig)

    if args.ram:
        print_tree('RAM', filter_symbols(lambda sec: sec.occupies_ram()))
        if args.show_treemap:
            fig = show_treemap('RAM', filter_symbols(lambda sec: sec.occupies_ram()))
            figs.append(fig)
    ...
    if args.use_sections:
        ...
        if args.show_treemap:
            fig = show_treemap(name, filter_symbols(lambda sec: sec.num in nums))
            figs.append(fig)
...
def get_figures(elf_file):
    figs = []
    if not os.path.isfile(elf_file):
        print('ELF file %s does not exist' % elf_file, file=sys.stderr)
        return figs
    ...
    # process symbols
    symbols = Symbol.extract_elf_symbols_info(elf_file, get_exe('readelf'))
    fileinfo = extract_elf_symbols_fileinfo(elf_file, get_exe('nm'))
    add_fileinfo_to_symbols(fileinfo, symbols)

    demangle_symbol_names(symbols, get_exe('c++filt'))
    # load section info
    sections = Section.extract_sections_info(elf_file, get_exe('readelf'))
    sections_dict = {sec.num: sec for sec in sections}

    def get_items_dicts(symbols):
        tree = SymbolsTreeByPath(symbols)
        tree.merge_paths(False)
        tree.sort(key=lambda symbol: symbol.size, reverse=True)
        return tree.get_tree_dict()
...
ERROR: No symbols from given section found or all were ignored!
    ...
    def get_treemap(header, symbols):
        items = {}
        items['size'] = []
        path = []
        maxdepth, itemdicts = get_items_dicts(symbols)
        for i in range(maxdepth + 1):
            items[i] = []
            path.append(i)
        ...
        for item in itemdicts:
            ...
            for i in range(maxdepth + 1):
                ...
                pass
        df = pd.DataFrame(items)
        fig = px.treemap(df, path=path, values = 'size', title = header)
        return fig

    figs = []
    figs.append(get_treemap('ROM Usage', filter_symbols(lambda sec: sec.occupies_rom())))
    figs.append(get_treemap('RAM usage', filter_symbols(lambda sec: sec.occupies_ram())))
